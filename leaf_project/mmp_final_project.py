...
if 'num_classes' in locals():
    ...
    if train_dl is not None:
        ...
        plt.plot(x_arr, hist[2], label='Train Accuracy')
        plt.plot(x_arr, hist[3], label='Validation Accuracy')
        ...
        plt.figure(figsize=(6, 5))
        plt.plot(x_arr, hist[0], label='Train Loss')
        plt.plot(x_arr, hist[1], label='Validation Loss')
        ...
        # Display final grid of predicted leaf samples
        print("\nFinal Visual Summary of Leaf Predictions:")
        leaf_grid_path = '/content/drive/MyDrive/RGB'
        # Assuming you want to display the first image found in the director
        for filename in os.listdir(leaf_grid_path):
          if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
            image_path = os.path.join(leaf_grid_path, filename)
            display(Image(filename=image_path))  # Display the image
          break  # Exit the loop after displaying one image
        else:
...
def visualize_predictions_custom():
    ...
    # Automatically find one image from dataset
    for root, dirs, files in os.walk('/content/drive/MyDrive/RGB'):
        for file in files:
